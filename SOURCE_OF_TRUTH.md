...
- **Follow-up correction (2026-08-19, third round, same day): the storm still wasn't stormy or distant enough -- a real eyewitness reference exposed both the visual AND the audio as wrong.** Player report, no screenshot this time, an actual lived reference instead: a real storm cell seen from roughly 50 miles away in Arizona, "extremely active... lightning lit up the sky constantly, lit up the clouds, and lightning bolts flew across the sky." Four specific complaints, all four addressed: (1) thunder audio "sounding like it's right at the player not way off in the distance"; (2) lightning frequency "very sparse" even after the second round's tightening; (3) `streak` reading as "only little lines not lightning"; (4) `cloud` doing "nothing that lights up the clouds." Root causes were different for each: (2)/(3) were pure numeric undershoot (`LIGHTNING_MIN/MAX_GAP_FRAMES` tightened again, 90-400 -> 25-140; `streak`'s span widened to 35-65% of canvas width with an added glow-then-core double stroke); (4) was a genuine rendering-technique bug, not a numbers problem -- the cloud glow and the shared sky-flash wash were both plain `source-over` alpha fills, which on a photo this dark pull the mountains' own dark tones TOWARD gray-white (desaturating) rather than adding brightness to them, so a real rendered screenshot of the "brighter/bigger blob" first attempt still read as fog smeared on the mountains, not a flash -- caught by actually looking at the render, not by trusting the larger numbers. Fixed by switching every flash element (`cloud`'s three blobs, the shared sky wash, and -- for consistency -- `streak`/`strike`'s own strokes and impact glow) to `ctx.globalCompositeOperation = 'lighter'` (additive/screen-style blending, each channel summed and clamped instead of mixed), which is what actually reads as light hitting a dark scene; (1) was an architecture gap, not a tuning knob -- `startEventAmbientLayer`'s signal chain (`source -> gain -> [panner] -> ambientGain`) had no filter node at all, so "distance" could only ever be simulated by lowering `gain`, which reads as "quiet but close" (the recording's full crack/crackle frequency content still intact) rather than "far away" (real distance rolls off high frequencies over range). Extended the function with an optional `lowpassHz` parameter (`source -> [filter] -> gain -> ...`, filter only inserted when a sound's own config asks for one, so every other event layer -- owl, gulls, whale, wildlife -- is untouched) and set `SCENE_AMBIENT_CONFIG.desert.sounds.thunder` to `{ gain: 0.24 (was 0.4), lowpassHz: 900 }`. Verified the same way as every round before it: real rendered screenshots of all three lightning kinds at production canvas size (1280x720) confirmed the additive blend actually produces a bright flash lighting the mountains/sky, not the earlier fog; two new tests cover the audio fix specifically -- one confirms a `BiquadFilterNode` (`type: 'lowpass'`, correct frequency) actually gets created and inserted into the chain when `lowpassHz` is configured, and gets created ZERO times for a sound that doesn't configure it (proving this is opt-in per-sound, not a blanket signal-chain change), the other pins `SCENE_AMBIENT_CONFIG.desert.sounds.thunder`'s own `lowpassHz`/`gain` values directly so a future edit can't silently regress either number. Full existing Desert suite (10 tests) and the full 256-test suite both re-run clean after the change. Screenshot capture itself hit a real methodology trap worth recording: an element screenshot of `#gameCanvas` (Playwright `locator().screenshot()`) captured the title-screen DOM overlay stacked on top of the canvas instead of the canvas's own bitmap, and a naive two-step `evaluate()` (set state, THEN read `canvas.toDataURL()` in a separate round-trip) let the game's own running `requestAnimationFrame` loop decrement `flashLife` (or re-roll `lightning.kind` entirely) between the two calls -- both fixed by reading `canvas.toDataURL()` directly, in the SAME `evaluate()` call that set up the state and called `drawDesertScene()`, with no intervening awaited round-trip for the real loop to interleave into.
- **Follow-up correction (2026-08-19/20, fourth round): the third round's numbers were still measured wrong -- and a PR reviewer caught two real regressions the third round's own fixes introduced.** Player, having actually played the third round's build rather than a formula: "I saw the clouds lighting up... needs more activity." Root cause: `LIGHTNING_MIN/MAX_GAP_FRAMES`'s gap is measured from one flash's own START to the next flash's START, not end-to-start -- with flash durations of only ~10-24 frames against a 25-140 frame gap, most of each cycle was genuinely empty sky even though the gap number itself already matched the player's earlier "once every second or two" framing. Tightened to 8-55 frames and lengthened `maxFlashLife` (cloud 24-34, streak/strike 16-24, both up from their third-round values) so overlapping flashes -- one still fading as the next starts -- become common instead of a near-miss; measured post-fix at ~67% of frames having an active flash over a 6000-frame simulated run, with roughly one overlap-start every ~2s. Separately, Codex's PR review on this same round caught two real bugs in the THIRD round's own code, neither only theoretical: (P2) `streak`'s span was clamped only on its second endpoint (`x2`) against the [0.03, 0.97] screen edges, while its direction came from `boltXFrac2`'s stored sign -- chosen independently of where `boltXFrac` actually sat -- so a start point near one edge paired with a direction pointing further toward that same edge collapsed the whole span to as little as 7% of canvas width, the exact "little line" regression that round existed to fix; fixed by picking direction from actual available room on each side and capping the span to whatever that side has, which guarantees at least ~35% of width every time (worst case is a dead-center start, ~47% each side) -- `boltXFrac2` was otherwise dead for `streak` once this landed, so removed entirely rather than left as an inert field. (P1, the more serious one) the enlarged `cloud` glow blobs (`blobR` up to `(0.32 + 0.14) * w`, e.g. 1229px at 3840×1080) were filled with no clip at all, so on any canvas shape where `skyBandH` is smaller than `blobR` -- common on wide/ultrawide -- the additive glow could bleed straight past `groundY` into the photographed ground, breaking the rule that only `strike` is deliberately allowed to touch that line; fixed with a hard rectangular clip to the real sky band applied before any blob is filled, verified both visually (an ultrawide render at max `boltDepthFrac` shows a sharp cutoff exactly at the ground line, no wash below it) and with a new pixel-level test that renders `cloud` at peak intensity vs. an identical flash-off frame across the same three-canvas-shape/five-phase sweep the general containment tests use and asserts the row just below the REAL `groundY` (read the same way the `strike` containment test reads it -- via a hooked `drawDesertCutout` call, not a formula re-derived in the test that could share a bug with the code under review) is byte-for-byte identical between the two. Both review fixes and the frequency/duration retune landed in the same PR before merge, not as a follow-up round. Full Desert suite (13 tests) and full 257-test suite both re-run clean.
- **Test result (2026-08-20): Venice, the Required Method applied proactively again, second time this method has been used for a scene built in one pass rather than reactively after a report.** Player request, verbatim: "the gondola area in front of st marks square in Venice Italy." Every category was addressed before the first render: (1) contact — the one cutout (`gondola.webp`) checked for real (>150 alpha) contiguous opaque coverage at its own bottom edge before being wired in; a first crop attempt used PIL's default `getbbox()` (any non-zero alpha counts), which included an essentially-invisible soft-shadow feather below the boat's real hull edge — caught by the project's own automated contact test (0px of real coverage measured), not a visual read, and fixed by re-cropping to a 150-alpha-threshold mask's bbox instead (178px of real coverage after the fix); (2) medium legibility — `VENICE_CONFIG.WATER_TOP_FRAC` (0.51) and `WATER_END_FRAC` (0.80) measured with a full-row brightness scan AND a visual reference-line overlay before being written into game.js — the scan alone was noisier here than Desert's single clean transition (the photographed gondolas' own near-black hulls sit inside the water band and skew a pure brightness read), and an initial scan-only guess (0.44) turned out to still be inside the shoreline-silhouette band, not the real water line — only caught by actually drawing the line and looking; (3) depth order — gondolas carry their own `yFrac` and are sorted by it every frame before drawing, same idiom as Desert's flora (one shared depth slot, since there's only one foreground element type here); (4) off-canvas justification — pigeons cross the screen and wrap, reusing Safari's own `generateSafariBirds`/`drawSafariScene` technique directly; (5) composition matches anchor role — the gondola cutout was picked specifically for its near-level, eye-height camera angle over an available alternate shot from a much steeper top-down/bridge angle, which would have read as a mismatched perspective against this scene's own eye-level background photo — checked BEFORE sourcing the asset, not discovered as a defect after; (6) region containment — every gondola's own anchor point (not just its rendered bounding box) is interpolated strictly between `waterTopY` and `waterEndY` by construction, both clamped the same way Beach's `horizonY`/Desert's `groundY` are (kept within the screen's middle 70% via the `panY` clamp, `waterEndY` additionally backstopped against the canvas's own bottom edge); pigeons (plain screen-space, not photo-anchored) are separately clamped to stay above `waterTopY` with a safety margin, the same fix Beach's own `celestialY` needed against the identical clamp; (7) compound placement — the yFrac sort handles arbitrary overlaps by construction, verified both structurally (a deliberately interleaved mix of yFracs never draws out of order) and with a literal same-x pin (a far and a near gondola forced to the same x, far one must draw first); (8) relative scale plausibility — `GONDOLA_SIZE_FRAC` is a single named range depth-scaled 0.4x-1.0x by `yFrac`, same technique as Beach's dolphins/Desert's flora. Pigeons are deliberately procedural, not a photo cutout, mirroring Safari's own birds directly rather than treating that as a downgrade from the real-photo direction — a whole-object photo of a bird mid-flight with nothing cropped at the frame edge is a much harder sourcing ask than a ground-standing animal, and this codebase already has an established, working answer for exactly that case; the signature real-photo element is the gondola itself. All eight categories have a corresponding automated test in `tests/smoke.spec.js`'s "VENICE SCENE" section (structural draw-order hook, same-x pin, geometry/canvas-shape sweep, deterministic depth-scale check, off-canvas wrap check), plus the shared cross-scene "Photo boundary fractions" and "every cutout touches its own bottom edge" tests were both extended to cover Venice's own constants/asset rather than given a parallel one-off test. Verified visually across three canvas shapes (portrait mobile, 1280×720, and 3840×1080 ultrawide) at production size before shipping — the ultrawide render in particular confirmed the cutout gondolas blend naturally alongside the background photo's own already-moored gondolas rather than reading as a separate, mismatched layer. Full 262-test suite passes.
- **PR review corrections (2026-08-20, same PR, before merge): Codex caught two real measurement/coverage gaps in the initial Venice build.** (P1) The real photographed gondola row's hulls extend to relative yFrac ~0.50 within the water band (measured directly: their darkest/lowest point sits at image-fraction 0.68), but the initial animated-gondola placement range (0.45-0.95) let some instances represent boats at the SAME or FARTHER depth as that real row — since an animated cutout always paints on top of the already-rasterized background regardless of its own yFrac, this produced boats that should have read as farther away visually occluding the real nearer ones, a category-3/5 violation no depth sort inside the animated set could fix (the photographed boats aren't part of that sortable set). Fixed by raising the yFrac floor to 0.62, a real measured margin above the row's own ceiling, not a guessed bump — verified with a new test that samples 500 generated scenes and asserts every single gondola's yFrac clears that ceiling with margin, not just on average. (P2) `WATER_END_FRAC` (0.80) was itself still wrong despite the proactive measurement process category 2 requires — the zoomed reference-line crop used to "confirm" it had itself started AT 0.80, begging the question rather than checking it. A wider brightness scan (0.78-0.89) found the real transition is a sharp, plateauing jump at 0.85-0.86 (110→144, then flat), not the gradual wave/foam climb 0.80 sat inside. Corrected to 0.85, with the "Photo boundary fractions" test's own threshold raised to match the real (much stronger) signal. Also fixed, proactively while investigating: the region-containment test only ever checked hooked anchors against the full canvas `[0, h]`, which structurally cannot catch a wrong `WATER_TOP_FRAC`/`WATER_END_FRAC` — a mismeasured constant still comfortably fits inside canvas bounds, which is exactly how the 0.80 error passed that test the first time. Fixed by extracting the pan/zoom/water-band math into a standalone `computeVeniceWaterBand(scene, w, h)` function that both `drawVeniceScene` and the test now call — the test gets the REAL, authoritative boundary for the exact frame it's checking, not a formula re-derived in the test that could share a bug with the code under review. Recorded here as a general lesson: a reference-line crop that already excludes the range under consideration isn't independent confirmation, and a containment test bounded by the canvas alone doesn't prove containment within the PHOTO'S mapped content, only within the screen. Full 262-test suite re-passes (1 known pre-existing, unrelated timing flake).

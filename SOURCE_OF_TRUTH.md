...
- **Open (2026-08-19), queued for the next audit pass**: Safari, player report/screenshot -- animals rendering on top of trees they should be behind (floating over foliage instead of standing in front of/beside it), and trees rendering underneath other trees in a way that reads as physically wrong (one tree nested inside another's canopy rather than a plausible near/far pair). Not yet investigated or fixed -- Safari predates the Required Method's depth-order category (`BEACH_DEPTH_LAYERS`-style explicit array + hookable draw functions) entirely; `SAFARI_CUTOUT_SOURCES`/`drawSafariCutout` were built before that category existed (see this file's own history: the depth-order category was added AFTER Beach's cruise-ship/palm draw-order bug, well after Safari shipped), so Safari's trees/animals likely still draw in whatever order `generateSafariScene`'s arrays happen to iterate, not a deliberate depth model. Player has explicitly requested a full Required Method pass on Safari next, after the current Desert storm correction ships -- treat as the next audit target, not a quick patch: apply every category (contact, medium legibility, depth order, off-canvas justification, composition-matches-anchor-role, region containment, compound placement, relative scale plausibility) proactively, the same standard Desert was just held to, not just the two specific symptoms in this report.

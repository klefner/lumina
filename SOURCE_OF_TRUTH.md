...
- **Resolved (was open, 2026-08-18)**: Beach's `palm-overhang` (the corner-hanging frond) failed the Required Method's category-4 check (off-canvas justification) — its trunk visibly terminated in open air well inside the frame rather than near/at a screen edge, the same underlying failure as round 2 above wearing a different asset. Introduced when `overhang.sizeFrac` was shrunk (0.55–0.70 → 0.28–0.36) to fix an unrelated "too large, blocks dots" complaint; nobody re-verified the "trunk exits the frame" justification still held at the smaller size (player report, screenshot). Root cause turned out to be structural, not just a bad size pick: the trunk's cutoff point sits at a fixed position relative to the crown, proportional to `sizeFrac`, so shrinking the cutout pulls that point toward the screen's open interior at ANY size well under "fills most of the screen" — there is no `sizeFrac` in the "not too large" range that fixes this by itself. Fixed by cropping the trunk out of the source image entirely (`art/beach-cutouts/palm-overhang.webp` is now crown/fronds only) rather than trying to tune a size or position — a dense foliage mass hanging into frame from a corner reads as "the rest of the tree is out of frame" without needing any limb shown at all. Confirmed by rendering at the full production `sizeFrac` range (both corners, min/mid/max) and inspecting a crop of the result each time, not by reasoning about the geometry alone. Category 4 still has no automated check (the geometry depends on where in a given source image the deliberate cut point falls, which isn't recorded anywhere) — this fix sidesteps that gap for this one asset by removing the element that needed the check, it doesn't close the gap itself. Building a real automated check for this category (recording an asset's cut-point fraction when first processed, then asserting it stays within some edge margin across the size range) is still open work if a future corner/edge-cropped asset needs one.
